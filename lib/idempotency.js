...
function withIdempotency(store) {
  return async (req, res) => {
          ...
          if (prev.request_fingerprint !== fp) {
            return res.status(409).json({
              message: 'Idempotency-Key sudah dipakai untuk payload berbeda.',
                    ...
                    let replayBody = prev.response_body;
          try { replayBody = typeof replayBody === 'string' ? JSON.parse(replayBody) : replayBody; } catch { /* keep */ }
          return res.status(prev.response_status).json(replayBody);
        }
      } catch (err) {
    ...
    if (key && result && result.status < 500) {
      // Simpan SINKRON supaya replay berikutnya konsisten (menghindari race
      // lookup-before-insert di serverless cold start). Kegagalan store tidak
      // menggagalkan request — fail-open.
      try {
        await db.query(
        ...
        // Cleanup bounded: hanya baris expired (index expires_at), fire-and-forget.
        db.query(`delete from idempotency_keys where expires_at < now()`).catch(() => {});
      } catch (err) {
        console.error('[idempotency store]', err.message);
      }
    }
