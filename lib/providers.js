// Downloader provider layer (diekstrak dari routes/download.js di Phase 3 —
// VERBATIM, behavior identik: SSRF guard tetap di route, timeout/fallback/
...
// Timeout policy terpusat (Phase 5): semua provider fetch lewat
// resilience.providerFetch — bounded timeout, retry hanya transient,
// circuit breaker instance-local, health tracking.
const { providerFetch, ERR, providerError, classify } = require('./resilience');

const PLATFORMS = ['tiktok', 'youtube', 'facebook', 'instagram'];

// ---------------------------------------------------------------------------
// Capability matrix (Phase 5) — model internal provider.
// name: label logis (breaker/health key); platforms: yang didukung;
// priority: urutan fallback (kecil = duluan); timeouts per operasi.
// ---------------------------------------------------------------------------
const PROVIDERS = [
  {
    name: 'cobalt',
    platforms: ['youtube'],
    operations: ['video', 'audio'],
    timeoutMs: 12_000,
    priority: 1, // primer utk YouTube
    expectedErrors: [ERR.TIMEOUT, ERR.UNAVAILABLE, ERR.RATE_LIMITED],
  },
  {
    name: 'vidssave',
    platforms: ['youtube'],
    operations: ['video', 'audio'],
    timeoutMs: 12_000,
    priority: 2, // fallback YouTube (shorts kadang cuma di sini)
    expectedErrors: [ERR.TIMEOUT, ERR.UNAVAILABLE, ERR.BAD_RESPONSE],
  },
  {
    name: 'ssstik',
    platforms: ['tiktok'],
    operations: ['video', 'audio', 'image'],
    timeoutMs: 12_000,
    priority: 1,
    expectedErrors: [ERR.TIMEOUT, ERR.UNAVAILABLE, ERR.BAD_RESPONSE],
  },
  ...
  },
  {
    ...
    platforms: ['instagram'],
    operations: ['video', 'image'],
    timeoutMs: 10_000,
    priority: 1, // graphql web publik
    expectedErrors: [ERR.RATE_LIMITED, ERR.UNAVAILABLE, ERR.BAD_RESPONSE],
  },
  {
    name: 'instagram-api',
    platforms: ['instagram'],
    operations: ['video', 'image'],
    timeoutMs: 10_000,
    priority: 2, // fallback api web resmi (app-id publik)
    expectedErrors: [ERR.RATE_LIMITED, ERR.UNAVAILABLE],
  },
];

function supports(providerName, platform) {
  const p = PROVIDERS.find((x) => x.name === providerName);
  return !!p && p.platforms.includes(platform);
}
...
// Pesan yang boleh sampai ke client per kategori — TANPA detail internal
// (nama provider mentah, URL internal, stack, dsb tetap hanya di log server).
const CATEGORY_MESSAGES = {
  [ERR.TIMEOUT]: 'Sumber data tidak merespons tepat waktu. Coba lagi sebentar lagi.',
  [ERR.UNAVAILABLE]: 'Sumber data sedang tidak tersedia. Coba lagi beberapa saat lagi.',
  [ERR.BAD_RESPONSE]: 'Sumber memberikan respons yang tidak bisa diproses. Konten mungkin privat atau dihapus.',
  [ERR.RATE_LIMITED]: 'Sumber sedang membatasi permintaan. Coba lagi beberapa menit lagi.',
  [ERR.UNSUPPORTED]: 'Platform atau URL tidak didukung.',
  [ERR.DOWNLOAD_FAILED]: 'Gagal mendapatkan tautan unduhan untuk URL ini.',
};

function fail(platform, url, category, status = 502, userMessage) {
  const cat = CATEGORY_MESSAGES[category] ? category : ERR.DOWNLOAD_FAILED;
  const e = new Error(cat);
  e.status = status;
  e.category = cat;
  // payload legacy tetap berbentuk {ok, platform, message} — message aman.
  e.payload = { ok: false, platform, url: url || null, message: userMessage || CATEGORY_MESSAGES[cat], media: [] };
  return e;
}
...
async function tiktok(url) {
  ...
  let html;
  try {
    const r = await providerFetch('ssstik', 'https://ssstik.io/abc?url=dl', {
      method: 'POST',
      headers: {
    ...
    throw fail('tiktok', url, classify(e), e.status || 502);
  }
  ...
  if (media.length === 0) {
    throw fail('tiktok', url, ERR.PROVIDER_BAD_RESPONSE, 502,
      'Tidak ada tautan unduhan. Video mungkin privat atau dihapus.');
  }
...
async function cobaltYouTube(rawUrl, downloadMode) {
  ...
  const r = await providerFetch('cobalt', COBALT, {
    method: 'POST',
    headers: {
  ...
  const err = new Error('cobalt error: ' + (code || d.status || r.status));
  // klasifikasikan utk retry/breaker: 429 dari cobalt = rate limited
  if (r.status === 429 || code === 'rate_limited') err.status = 429;
...
async function youtube(rawUrl) {
  ...
  let data;
  try {
    const r = await providerFetch('vidssave',
    ...
    throw fail('youtube', rawUrl, classify(e), e.status || 502);
  }

  if (!data || data.status !== 1 || !data.data) {
    throw fail('youtube', rawUrl, ERR.PROVIDER_BAD_RESPONSE, 502,
      'URL ini tidak bisa diproses (mungkin bukan video publik).');
  }
  ...
  if (media.length === 0) {
    throw fail('youtube', rawUrl, ERR.DOWNLOAD_FAILED, 502,
      'Tidak ada sumber unduhan untuk video ini.');
  }
...
async function facebook(rawUrl) {
  ...
  let html;
  try {
    const r = await providerFetch('snapsave', 'https://snapsave.app/action.php?lang=en', {
      method: 'POST',
      headers: {
        'content-type': 'application/x-www-form-urlencoded',
        'user-agent': UA,
    ...
    throw fail('facebook', rawUrl, classify(e), e.status || 502);
  }
  ...
  if (/error_api_get|Error:/.test(html) && !/rapidcdn|download/i.test(html)) {
    ...
    throw fail('facebook', rawUrl, ERR.PROVIDER_BAD_RESPONSE, 502, msg || 'Video privat atau tidak ditemukan.');
  }
  ...
  if (media.length === 0) {
    throw fail('facebook', rawUrl, ERR.PROVIDER_BAD_RESPONSE, 502,
      'Tidak ada tautan unduhan. Video mungkin privat, bukan video, atau dihapus.');
  }
...
async function instagram(rawUrl) {
  ...
  if (!shortcode) {
    throw fail('instagram', rawUrl, ERR.PROVIDER_UNSUPPORTED, 400,
      'URL Instagram tidak dikenali. Pakai format instagram.com/p/<kode> atau /reel/<kode>.');
  }
  ...
  let data = null;
  try {
    const r = await providerFetch('instagram-graphql', gqlUrl, {
      headers: {
        'user-agent': UA,
        'x-requested-with': 'XMLHttpRequest',
  ...
  if (!data) {
    try {
      const r = await providerFetch('instagram-api',
  ...
  if (!data) {
    throw fail('instagram', rawUrl, ERR.PROVIDER_RATE_LIMITED, 502,
      'Sumber Instagram menolak permintaan dari server ini (rate-limit / login wajib). Coba lagi beberapa menit lagi.');
  }
  ...
  if (media.length === 0) {
    throw fail('instagram', rawUrl, ERR.DOWNLOAD_FAILED, 502,
      'Media tidak ditemukan untuk URL ini.');
  }
...
module.exports = { PLATFORMS, PROVIDERS, supports, ok, fail, tiktok, youtube, facebook, instagram, CATEGORY_MESSAGES };
